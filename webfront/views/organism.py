...
class ProteomeAccessionHandler(CustomView):
    ...
    def get(
        ...
        **kwargs
        ...
        general_handler.queryset_manager.add_filter(
            "proteome", accession=endpoint_levels[level - 1].lower()
        )
    ...
    def filter(queryset, level_name="", general_handler=None):
        general_handler.queryset_manager.add_filter(
            "proteome", accession=level_name.lower()
        )
        return queryset
    ...
    def get(
        ...
        **kwargs
        ...
        if "accession" in general_handler.queryset_manager.filters["taxonomy"]:
            tax_id = general_handler.queryset_manager.remove_filter(
                "taxonomy", "accession"
            )
            general_handler.queryset_manager.add_filter(
                "proteome", taxonomy__lineage__contains=" {} ".format(tax_id)
            )
        # if level == 2:
...
class TaxonomyAccessionHandler(CustomView):
    ...
    many = False
    child_handlers = [("proteome", ProteomeHandler)]
    serializer_detail_filter = SerializerDetail.ORGANISM_DETAIL
    ...
    def get(
        ...
        **kwargs
        ...
        general_handler.queryset_manager.add_filter(
            "taxonomy", accession=endpoint_levels[level - 1].lower()
        )
        general_handler.modifiers.register(
    ...
    def filter(queryset, level_name="", general_handler=None):
        general_handler.queryset_manager.add_filter(
            "taxonomy", accession=level_name.lower()
        )
        return queryset
...
class TaxonomyHandler(CustomView):
    ...
    child_handlers = [(r"\d+", TaxonomyAccessionHandler), ("proteome", ProteomeHandler)]
    queryset = Taxonomy.objects.all()
    serializer_class = OrganismSerializer
...
class OrganismHandler(CustomView):
    ...
    from_model = False
    child_handlers = [("taxonomy", TaxonomyHandler), ("proteome", ProteomeHandler)]
    many = False
    serializer_class = OrganismSerializer
